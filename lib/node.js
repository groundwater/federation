...
Node.prototype.ask     = function(address,question,timeout){
  if(!timeout) timeout = DEFAULT_TIMEOUT;
  var anon = this.router.createNodeWithExpiry(null,timeout);
  anon.tell(address,question);
  var _enq = anon.enqueue;
  anon.enqueue = function(packet){
    clearTimeout(anon.timer);
    _enq.call(anon,packet);
  }
  return anon;
}
