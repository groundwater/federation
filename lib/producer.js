...
ProducerForge.prototype.NewWithRouterAndNode = function(router,node){
  ...
  emitter.on('enqueue',function(toPath,fromPath,message,count){
  ...
  emitter.on('dropped',function(toPath,fromPath,message){
    console.warn('Message Dropped from %s to %s',fromPath,toPath);
  });
  ...
  node.receive = function(packet){
    ...
    director.enqueue(fromPath,toPath,message,count);
  }
