...
Router.prototype.createNodeWithExpiry = function(name,timeout){
  assert(timeout,'Expiring Nodes Require Timeout');
  var self = this;
  var node = this.createNode(name);
  var path = node.name;
  
  node.timer = setTimeout(function(){
    self.nodes[path]._error('TIMEOUT');
    delete self.nodes[path];
  },timeout);
